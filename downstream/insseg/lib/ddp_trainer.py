...
from models.spconv_unet import SpUNetBase
from lib.test import test as test_
from lib.solvers import initialize_optimizer, initialize_scheduler
from datasets import load_dataset
from datasets.dataset import initialize_data_loader
from lib.utils import checkpoint, precision_at_one, Timer, AverageMeter, get_prediction, load_state_with_same_shape, \
    count_parameters
...
class SegmentationTrainer:
    def __init__(self, config):
        ...
        model = SpUNetBase(in_channels=num_in_channel,
                           out_channels=num_labels,
                           channels=(32, 64, 128, 256, 256, 128, 64, 64),
                           layers=(2, 3, 4, 6, 2, 2, 2, 2))
        logging.info('===> Number of trainable parameters: {}: {}'.format("SpUNetBase", count_parameters(model)))
        logging.info(model)
        ...
        if config.net.weights != '':
            ...
            load_state_info = model.load_state_dict(matched_weights, strict=False)
            logging.info(f"Missing keys: {load_state_info[0]}")
        ...
        while self.is_training:
            for _ in range(len(self.train_data_loader) // self.config.optimizer.iter_size):
                ...
                for sub_iter in range(self.config.optimizer.iter_size):
                    ...
                    pt_offsets, soutput = self.model(coords.cuda(), input.cuda())
                    # The output of the network is not sorted
                    target = target.long().to(self.cur_device)
                    semantic_loss = self.criterion(soutput, target.long())
                    ...
                    pred = get_prediction(self.train_data_loader.dataset, soutput, target)
                    ...
                    # gather information
                    logging_output = {'total_loss': total_loss.item(), 'semantic_loss': semantic_loss.item(),
                                      'score': score / self.config.optimizer.iter_size}
                ...
                if self.curr_iter % self.config.train.stat_freq == 0 or self.curr_iter == 1:
                    ...
                    debug_str += "Score {:.3f}\tData time: {:.4f}, Forward time: {:.4f}, Backward time: {:.4f}, DDP time: {:.4f}, Total iter time: {:.4f}".format(
                        scores.avg, data_time_avg.avg, fw_time_avg.avg, bw_time_avg.avg, ddp_time_avg.avg,
                        iter_time_avg.avg)
